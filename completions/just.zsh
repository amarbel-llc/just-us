...
'--output-format=[Set output format\: '\''default'\'' for human-readable, '\''tap'\'' for TAP version 14]:FORMAT:(default tap)' \
'--tap-stream=[Set TAP streaming mode\: '\''streamed-output'\'' (default) emits output as comments, '\''buffered'\'' collects in YAML blocks, '\''stderr'\'' copies to stderr]:MODE:(buffered streamed-output stderr)' \
