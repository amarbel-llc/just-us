...
complete -c just -l output-format -d 'Set output format: \'default\' for human-readable, \'tap\' for TAP version 14' -r -f -a "default\t''
tap\t''"
complete -c just -l tap-stream -d 'Set TAP streaming mode: \'streamed-output\' (default) emits output as comments, \'buffered\' collects in YAML blocks, \'stderr\' copies to stderr' -r -f -a "buffered\t''
streamed-output\t''
stderr\t''"
...
complete -c just -l agents-only -d 'Only allow running recipes marked [agents(\'always-allowed\')]'
